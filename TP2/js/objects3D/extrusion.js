...
class ExtrusionSurface {
    ...
    extrude() {
        ...
        for (let i = 0; i < path_points.length; i++) {
            ...
            B = path_binormal[i];
            T = path_tangent[i];
            // Construir la matriz de nivel en un punto de la curva de camino.
            levelMatrix = mat4.fromValues(N[0], N[1], N[2], 0, B[0], B[1], B[2], 0,
                                        T[0], T[1], T[2], 0, P[0], P[1], P[2], 1);
            // Construir la matriz que rota las normales (no incluye traslaciones).
            normalMatrix = mat4.fromValues(N[0], N[1], N[2], 0, B[0], B[1], B[2], 0,
                                        T[0], T[1], T[2], 0, 0, 0, 0, 1);
            ...
            for (let j = 0; j < design_points.length; j++) {
                ...
                // La normal obtenida apunta hacia adentro en las discretizaciones
                // pero para la iluminación se necesita saliente.
        ...
        let cols = this.n_path - 1;
        let rows = this.n_design - 1;
